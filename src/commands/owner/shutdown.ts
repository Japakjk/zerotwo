...
export default {
  data: new SlashCommandBuilder()
    .setName('shutdown')
    .setDescription('Desliga o bot (Apenas para o Owner do Bot).'),
  async execute(interaction: ChatInputCommandInteraction) {
    if (!OWNER_IDS.includes(interaction.user.id)) {
      return interaction.editReply({ content: `${Emojis.ban} Apenas o **Owner do Bot** pode usar este comando!` });
    }
    await interaction.editReply({ content: `${Emojis.ban} Desligando o bot... Até logo, Darling! 🦖` });
    process.exit(1);
  },
  async executeText(message: Message) {
    ...
    await message.reply(`${Emojis.ban} Desligando o bot... Até logo, Darling! 🦖`);
    process.exit(1);
  }
};
