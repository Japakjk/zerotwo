...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: flertar`);

    const embed = SocialService.getInteractionEmbed('flertar', interaction.user.username, target.username);
