...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: cafune`);

    const embed = SocialService.getInteractionEmbed('cafune', interaction.user.username, target.username);
