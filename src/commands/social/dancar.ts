...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: dancar`);

    const embed = SocialService.getInteractionEmbed('dancar', interaction.user.username, target.username);
