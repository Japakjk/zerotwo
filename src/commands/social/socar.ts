...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: socar`);

    const embed = SocialService.getInteractionEmbed('socar', interaction.user.username, target.username);
