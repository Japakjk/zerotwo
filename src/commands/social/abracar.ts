...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: abracar`);

    const embed = SocialService.getInteractionEmbed('abracar', interaction.user.username, target.username);
