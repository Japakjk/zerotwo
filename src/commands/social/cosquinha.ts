...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: cosquinha`);

    const embed = SocialService.getInteractionEmbed('cosquinha', interaction.user.username, target.username);
