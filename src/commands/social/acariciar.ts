...
export default {
  data: new SlashCommandBuilder()
    .setName('acariciar')
    .setDescription('Faça um carinho em um Darling.')
    .addUserOption(opt => opt.setName('usuario').setDescription('O Darling que você quer acariciar').setRequired(true)),

  async execute(interaction: ChatInputCommandInteraction) {
    const target = interaction.options.getUser('usuario', true);
    if (target.id === interaction.user.id) return interaction.editReply({ content: 'Você não pode se acariciar sozinho, Darling! 🦖🌸' });
  ...
  async executeText(message: Message, args: string[]) {
    ...
    if (target.id === message.author.id) return message.reply({ content: 'Você não pode se acariciar sozinho, Darling! 🦖🌸' });
