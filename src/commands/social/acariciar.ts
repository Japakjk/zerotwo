...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: acariciar`);

    const embed = SocialService.getInteractionEmbed('acariciar', interaction.user.username, target.username);
