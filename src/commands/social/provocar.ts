...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: provocar`);

    const embed = SocialService.getInteractionEmbed('provocar', interaction.user.username, target.username);
