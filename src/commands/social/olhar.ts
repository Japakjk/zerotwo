...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: olhar`);

    const embed = SocialService.getInteractionEmbed('olhar', interaction.user.username, target.username);
