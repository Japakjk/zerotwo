...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await EconomyService.addCoins(userId, guildId, reward, `Interação Social: beijar`);

    const embed = SocialService.getInteractionEmbed('beijar', interaction.user.username, target.username);
