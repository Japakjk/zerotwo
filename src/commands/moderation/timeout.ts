...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!durationMs || durationMs > 28 * 24 * 60 * 60 * 1000) {
      return interaction.editReply({ content: 'Duração inválida! O máximo é 28 dias.' });
    }

    try {
      await member.timeout(durationMs, reason);
      const modCase = await ModerationService.createCase(interaction.guildId!, target.id, interaction.user.id, 'timeout', reason, durationStr);
